...
% (ii) over 4,400 existing installations with $\sim 10,800$ BHEs.
...
% The extraction of shallow geothermal energy using ground-source heat pumps (GSHPs) of depths up to 400 m is growing worldwide. Switzerland is the country with the world’s highest density of shallow geothermal installations, most of which are GSHPs with vertical borehole heat exchangers (BHEs) \cite{link_statistik_2019}. A dense installation of BHEs may, however, lead to an excessive long-term cooling of the subsurface due to thermal interactions between boreholes. Thus, thermal interaction must be considered when quantifying the technical potential of BHEs, that is, the long-term maximum energy which can be extracted annually using GSHPs \cite{bayer_geothermal_2019}. 
While analytical methods exist to quantify this potential, they are computationally intensive and require the availability of high-resolution data of the thermal properties of the shallow ground. These limitations may hinder the use of analytical models to quantify the technical potential of shallow GSHPs at national scale. By contrast, Machine Learning (ML) can handle missing data in analytical models and, once trained, is computationally highly efficient \cite{assouline_machine_2019-1}.  % CHECK DAN REFERENCES

Here we use ML in combination with geospatial tools for data pre-processing to estimate the technical potential of BHEs. 
...
The geological properties, which include the thermal conductivity $\lambda$ (at depths of 50, 100, 150, and 200 m) and thermal diffusivity $\alpha$ of the ground material, should reflect the thermal characteristics found across Switzerland. 
...
\multirow{2}{*}{\textit{\begin{tabular}[c]{@{}l@{}}Borehole\\ arrangements\end{tabular}}} & $A_{field}$ & Available area of building parcel & Fields in VD/GE & Section \ref{results} \\
 & $S_{10,25,50}$ & Surrounding avail. area in 10, 25, 50 m & Fields in VD/GE & Section \ref{results} \\ \hline
\multirow{2}{*}{\textit{\begin{tabular}[c]{@{}l@{}}Geological\\ properties\end{tabular}}} & $\lambda_{50-200}$ & Th.   conductivity, at depths $50-200$ m & 1   – 5.3 W/mK & SIA   \cite{sia_sondes_2010} \\
 & $\alpha$ & Thermal   diffusivity (average) & $0.6-1.4$ $\mu$m$^2$/s & Section \ref{results} \\ \hline
\multirow{3}{*}{\textit{\begin{tabular}[c]{@{}l@{}}Meteorological\\ conditions\end{tabular}}} & $T_0 $ & Surface temperature, 1m below ground & $7.5-11$ °C & Assouline   et al. \cite{assouline_machine_2019-1} \\
...
\multirow{4}{*}{\textit{Targets}} & $Q_{pot}$ & Energy   potential (kWh/a) & ML   output &  \\
 & $q_{max}$ & Heat extraction rate (W/m) & ML output &  \\
 & $N_{BHE}$ & Number of installed boreholes & ML output &  \\
 & $H$ & Borehole   depth (m) & \multicolumn{2}{l}{Computed as $H=\frac{Q_{pot}}{q_{max}\ t_{op}\ N_{BHE}}$ (Eq.~\ref{eq:Q_field})} \\ \hline
\end{tabular}
\end{table}

\begin{figure}[tb]
\centering\includegraphics[width=0.4\linewidth]{images/Figs/surr_buff_example.pdf}
...
For the classification, the same model (RF, FTR) was selected and tuned. As the classes in this classification task are unbalanced (there are many more suitable fields than unsuitable ones), we use the average f1-score as performance evaluation metric (see Chapter~\ref{ML_models}).
...
In addition to the three variables analysed previously, we compute $H$, the borehole depth, as the fourth variable representing the borehole arrangement. The computed $H$ is consequently no longer in the discrete set used in the simulation ($50-200$ m), but instead is a continuous variable. Results show that 92.7\% of $H$ are in the simulated range of $50-200$ m, suggesting a good physical consistency between the estimated target variables.
